		...
		if(mysqli_num_rows($data_query) > 0) {
			...
			while($row = mysqli_fetch_array($data_query)) {
				...
				if($user_logged_obj->isFriend($added_by)){
					...
					$s = mysqli_query($this->con, "SELECT likes, added_by FROM posts WHERE id='$id'");
					$k = mysqli_fetch_array($s);
					$u = $k['likes']; 
					...
					$str .= "
							<div class='post'>
								<div class='boosted-icon' data-toggle='tooltip' title='' data-original-title='Promoted'>
								...
								<div class='status_post' onClick='javascript:toggle$id()'>
									...
									<div class='post_header' style='display:flex'>
										<div class='post_avatar' >
											<img src='$profile_pic' class='' width='50' style='border-radius:50%'>
										</div>
										<div class='post_meta' >
											<span data-type='user' data-uid='' class='js_user-popover'>
												<a class='post_author' href='$added_by'>$first_name $last_name </a>
												$delete_button
											</span>
											<div class='post-time' style='margin-top:10px'>
												<a href='' class='js_moment' data-time='2021-05-07 15:18:26' title='Friday, May 7, 2021 10:18 pm'>$time_message</a>
												<i class='fa fa-globe' data-toggle='tooltip'></i>
											</div>
										</div>
									</div>
									
									<div id='post_body' style='font-size: 15px;line-height: 22px;'>
										<div class='post_text' dir='auto'>
											<textarea readonly  class='post_textarea'>$body</textarea>
										</div>
									</div>

									<div class='post_stats'>
										<div class='' style='margin-right: 10px!important;float:left!important;'>
											<div style='display: flex;cursor: pointer;line-height: 24px;'>
												<div style='height: 20px;width: 20px;display: flex;justify-content: center;align-items: center;background: #3367d6;border-radius:50%' >
													<i class='fa fa-thumbs-up' style='color:#fff'></i>
												</div>
												<span style='font-size:1.5rem;margin-left:5px'>$u</span>
											</div>
										</div>
										<div style='float: right!important;box-sizing: border-box;'>
									...
									<div style='margin-top: 10px;
											...
											border-top: 1px solid #f4f4f4;display: block;
											clear: both;
											display:flex;justify-content:center'>
											<iframe src='like.php?post_id=$id' scrolling='no' style='border:0;height:20px'>
											</iframe>
											<div>Comment</div>
									</div>
								</div>
		...
		if(mysqli_num_rows($data_query) > 0) {
			...
			while($row = mysqli_fetch_array($data_query)) {
				...
				$date_time = $row['date_added'];
					...
					$comments_check = mysqli_query($this->con, "SELECT * FROM comments WHERE post_id='$id'");
					$comments_check_num = mysqli_num_rows($comments_check);
					// $s = mysqli_query($con, "SELECT likes, added_by FROM posts WHERE id='$post_id'");
					// $k = mysqli_fetch_array($s);
					// $u = $k['likes']; 
